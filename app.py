...
# --- 5. 功能選單 (確保這部分一定會被執行) ---
menu = st.sidebar.selectbox("切換功能", ["🔍 查詢與修改", "🛠️ 老師管理後台"])
...
if menu == "🔍 查詢與修改":
    ...
    if not is_admin:
        ...
        st.sidebar.divider()
        if st.sidebar.button("🗑️ 刪除紀錄"):
            if not st.session_state.main_df.empty:
                target = st.selectbox("刪除作業：", ["請選擇"] + list(st.session_state.main_df["作業名稱"].unique()))
                if st.button("執行刪除") and target != "請選擇":
                    save_all(st.session_state.main_df[st.session_state.main_df["作業名稱"] != target])
                    st.rerun()

        if st.sidebar.button("🔄 強制刷新"):
            st.session_state.main_df = load_data(); st.rerun()
